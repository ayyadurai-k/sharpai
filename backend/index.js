...
const { calculateAndStoreDailyActiveUsers } = require("./controller/AdminController");
...
app.get('/redis-check', async (req, res) => {
    try {
        ...
        res.status(200).json({ status: true, message: 'Redis Connected Successfully', ping });
    } catch (error) {
        console.error('Redis Connection Error:', error);
        res.status(500).json({ status: false, message: 'Redis Connection Failed', error: error.message });
    }
});
