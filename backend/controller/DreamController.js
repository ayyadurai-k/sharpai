...
const saveRoadmap = async function (req, res) {
  try {
    ...
    res.status(201).json({ message: "Roadmap saved successfully test change" });
  } catch (error) {
...
const generateRoadmap = async (req, res) => {
  ...
  try {
    ...
    const prompt = `Analyze the following user's dream and career aspirations: ${JSON.stringify(
      { CareerAspiration, IndustryField, DesiredJobPosition }
    )}.
    ...
    if (
      ...
      return res
        .status(500)
        .json({
          message: "AI response did not contain a valid JSON structure.",
        });
    }
    ...
    if (parsedResponse && parsedResponse.nodes && parsedResponse.connections) {
      ...
      return res
        .status(400)
        .json({ message: "AI response is missing required data." });
    }
  } catch (error) {
    console.error("Error generating roadmap:", error);
    return res
      .status(500)
      .json({ message: "Internal Server Error during AI generation." });
  }
};
